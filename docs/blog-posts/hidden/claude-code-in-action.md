...
Tổng hợp chi tiết nội dung khóa học Claude Code in Action - hướng dẫn thực hành sử dụng Claude Code để tăng tốc quy trình phát triển phần mềm.
...
- Quy trình:
  1. Assistant gắn thêm chỉ dẫn vào yêu cầu của người dùng
  2. Chỉ dẫn quy định format phản hồi cho các hành động (ví dụ: "read file: filename")
  3. Language model phản hồi với yêu cầu hành động theo format
  4. Assistant thực thi hành động thực tế (đọc file, chạy lệnh)
  5. Kết quả được gửi lại cho language model để tạo phản hồi cuối cùng
